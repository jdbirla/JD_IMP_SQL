...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure BQ9RU_CB01_CLTBNK
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "Jd1dta"."BQ9RU_CB01_CLTBNK" (i_scheduleName   IN VARCHAR2,
  ...
  * MAR11    MKS       CB2   Performance Improvement
  *****************************************************************************************************/
  -----Local Vairables-----
  ...
  v_refnum_bankcc_check             VARCHAR2(100);
  ...
  ----------Constant--------------
  C_PREFIX       CONSTANT VARCHAR2(2) := Jd1dta.GET_MIGRATION_PREFIX('CLCB',
                                                              i_company);
  ...
  C_limit	   PLS_INTEGER := i_array_size; --CB2
  ...
  --getbankkey      PKG_COMMON_DMCB.bankkey; --CB2
  --checkclient     PKG_COMMON_DMCB.zdclpf_cp01; --CB2
  checkdupl       pkg_common_dmcb.cbduplicate;
  --getzigvalue     pkg_common_dmcb.zigvaluetype; --CB2
  --getclbapforcc   pkg_common_dmcb.clbatype; --Kevin: RUAT perf change
  ...
  CURSOR c_bankcursor IS
     select TIT.*,CLPF.ZENTITY as ZENTITY_DUP
      from Jd1dta.DMIGTITDMGCLNTBANK TIT
      left outer join
      (select *
      from Jd1dta.PAZDCLPF
     where prefix = 'CB') CLPF 
     on (TRIM(TIT.refnum) || '-' ||
                              TRIM(TIT.SEQNO) || '-' ||
                              concat(TIT.BANKCD, '   ' || TIT.BRANCHCD) || '-' ||
                              CASE WHEN RTRIM(TIT.BANKACCKEY) IS NOT NULL THEN
                              RTRIM(TIT.BANKACCKEY)
                              ELSE
                              RTRIM(TIT.CRDTCARD)
                              END) = CLPF.ZENTITY
     where RECIDXCLBK between start_id and end_id;


  obj_bank c_bankcursor%rowtype;
  TYPE t_clntbank_list IS TABLE OF c_bankcursor%rowtype;
  clnbnk_list t_clntbank_list;
...
BEGIN
  ...
  --PKG_COMMON_DMCB.getbankkey(getbankkey => getbankkey); --CB2: Add this logic to cursor for performance improvement
  --PKG_COMMON_DMCB.checkclient(checkclient => checkclient); --CB2: Add this logic to cursor for performance improvement
 -- pkg_common_dmcb.checkcbdup(checkdupl => checkdupl);
  --pkg_common_dmcb.getzigvalue(getzigvalue => getzigvalue); --CB2: Add this logic to cursor for performance improvement
  -- following package.proc added part of ITR3
  --pkg_common_dmcb.getclbapforcc(getclbapforcc => getclbapforcc);  --Kevin: RUAT perf change
  ---------Common Function Calling------------
  OPEN c_bankcursor;

  LOOP
	FETCH c_bankcursor BULK COLLECT INTO clnbnk_list LIMIT C_limit;
	<<again_start>>
	FOR i IN 1..clnbnk_list.COUNT LOOP

	obj_bank := clnbnk_list(i);
  /* CB2: Performance Improvement
    FETCH c_bankcursor
      INTO obj_bank;
    EXIT WHEN c_bankcursor%notfound;
  */
 -- DBMS_OUTPUT.PUT_LINE('REFNUM ' ||TRIM(obj_bank.REFNUM));
    ...
    IF TRIM(obj_bank.BANKACCKEY) IS NULL THEN
      ...
      v_refnum_bankcc_check:= TRIM(obj_bank.ZIGVALUE) ||v_bankkey||TRIM(obj_bank.CRDTCARD);
    ELSE
      v_refnum             := TRIM(obj_bank.refnum) || '-' ||
                              TRIM(obj_bank.SEQNO) || '-' ||
                              TRIM(v_bankkey) || '-' ||
      ...
      v_refnum_bankcc_check :=TRIM(obj_bank.ZIGVALUE)||v_bankkey||TRIM(obj_bank.BANKACCKEY);
    END IF;
    --DBMS_OUTPUT.PUT_LINE('v_bankacckeycrdtcard ' ||TRIM(v_bankacckeycrdtcard));
    ...
    ---Check client 
  --  DBMS_OUTPUT.PUT_LINE('Client check outside if condition'||checkclient(obj_bank.refnum));
   -- DBMS_OUTPUT.PUT_LINE('v_refnum '||v_refnum);
    --IF NOT (checkclient.exists(TRIM(obj_bank.refnum))) THEN -- CB2: perfomance improvement
	IF TRIM(obj_bank.ZENTITY) IS NULL THEN -- CB2: perfomance improvement
      v_isAnyError                 := 'Y';
    ...
    IF (obj_bank.ZENTITY_DUP IS NOT NULL) THEN
      v_isAnyError                 := 'Y';
      i_zdoe_info.i_indic          := 'E';
    ...
    END IF;
  -- 
  -- Following code developed part of ITR3 to check if bankacckey and crdt card details are already available in clbapf?
  -- 
  IF TRIM(obj_bank.BANKACCKEY) IS NOT NULL 
			AND (TRIM(obj_bank.CLB_CLNTNUM) IS NOT NULL) THEN --Kevin: RUAT perf change
		--DBMS_OUTPUT.PUT_LINE('INSIDE NEW CODE FOR BANKACCKEY ' ||	TRIM(v_refnum||obj_bank.BANKACCKEY));
	  v_isAnyError := 'Y';
      v_errorCount := v_errorCount + 1;
      t_ercode(v_errorCount) := C_RQO6;
      t_errorfield(v_errorCount) := 'BANKACCKEY';
      t_errormsg(v_errorCount) := o_errortext(C_RQO6);
      t_errorfieldval(v_errorCount) := v_refnum||obj_bank.BANKACCKEY;
      t_errorprogram(v_errorCount) := i_scheduleName;
  ...
  IF TRIM(obj_bank.CRDTCARD) IS NOT NULL 
			AND (TRIM(obj_bank.CLB_CLNTNUM) IS NOT NULL) THEN --Kevin: RUAT perf change
		--	DBMS_OUTPUT.PUT_LINE('INSIDE NEW CODE FOR CRDTCARD ' ||	TRIM(v_refnum||obj_bank.CRDTCARD));
		v_isAnyError := 'Y';
      v_errorCount := v_errorCount + 1;
      t_ercode(v_errorCount) := C_RQO6;
      t_errorfield(v_errorCount) := 'CRDTCARD';
      t_errormsg(v_errorCount) := o_errortext(C_RQO6);
      t_errorfieldval(v_errorCount) := v_refnum||obj_bank.CRDTCARD;
      t_errorprogram(v_errorCount) := i_scheduleName;
    ...
    --IF NOT (getbankkey.exists(v_bankkey)) THEN --CB2: add to cursor
	IF TRIM(obj_bank.BANKKEY) IS NULL THEN --CB2: add to cursor
      v_isAnyError := 'Y';
      v_errorCount := v_errorCount + 1;
    ...
    IF i_zprvaldYN = 'N' AND v_isAnyError = 'N' THEN
      /* CB3 Not Required as validation is already present
      ...
      END IF;
	  */
	  v_igvalue := TRIM(obj_bank.ZIGVALUE);
     ...
     begin
      ...
      v_unique_number := SEQ_CLBAPF.nextval; --PerfImprov
    ...
    exception
    when others then
    dbms_output.put_line ('err msg from clbapf insert'||sqlerrm);
    ...
    exception
    when others then
    dbms_output.put_line ('err msg from clrrpf insert'||sqlerrm);
    ...
    begin
      ...
      exception
    when others then
    dbms_output.put_line ('err msg from audit_clrrpf insert'||sqlerrm);

      end;

    END IF;
	END LOOP; --CB2
	EXIT WHEN c_bankcursor%notfound; --CB2
	COMMIT; --CB2
  END LOOP; --CB2
  COMMIT; --CB2
  CLOSE c_bankcursor;
