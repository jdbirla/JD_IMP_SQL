/*
  PA Data Migration G1ZDPOLCOV_CALL.sql
*/
...
select 'G1ZDPOLCOV' as SCHEDULENAME from dual;
column schedulenumber new_value v_schedulenumber noprint

SELECT 
   lpad(substr(nvl(table_name, 'ZDOEPC0000'), 7, 4) + 1, 4, 0) as schedulenumber
      FROM (SELECT table_name
              FROM all_tables
             WHERE owner = 'Jd1dta'
               AND table_name LIKE 'ZDOEPC%'
...
spool "&SQL_LOG_PATH.\G1ZDPOLCOV_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDPOLCOV';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Policy Coverage History';
  BEGIN
BEGIN
SELECT nvl(table_name, 'ZDOEPC0000')
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGMBRINDP2 PC;
/
...
BEGIN
 ...
 Jd1dta.PV_PC_G1ZDPOLCOV(i_schedulename =>I_SCHEDULENAME,
                 i_schedulenumber => V_SCHEDULENUMBER);
END;
/
