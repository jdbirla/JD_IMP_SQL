/*
  PA Data Migration G1ZDCOPCLT_CALL
*/
...
select 'G1ZDCOPCLT' as SCHEDULENAME from dual;
...
SELECT 
   lpad(substr(table_name, 7, 4) + 1, 4, 0) as schedulenumber
      FROM (SELECT table_name
              FROM all_tables
             WHERE owner = 'Jd1dta'
               AND table_name LIKE 'ZDOECC%' -- Data Migration - Ok/Error Table
...
spool "&SQL_LOG_PATH.\G1ZDCOPCLT_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDCOPCLT';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Corporate Client Testing by sql';
  BEGIN
BEGIN
SELECT table_name
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGCLNTCORP CC;
...
BEGIN
 ...
 Jd1dta.PV_CC_G1ZDCOPCLT(i_schedulename =>I_SCHEDULENAME,
                 i_schedulenumber => V_SCHEDULENUMBER);
END;
/
