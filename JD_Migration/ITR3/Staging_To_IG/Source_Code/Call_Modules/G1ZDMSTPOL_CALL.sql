/*
  PA Data Migration G1ZDMSTPOL_CALL
*/
...
select 'G1ZDMSTPOL' as SCHEDULENAME from dual;
...
SELECT 
   lpad(substr(table_name, 7, 4) + 1, 4, 0) as schedulenumber
      FROM (SELECT table_name
              FROM all_tables
             WHERE owner = 'Jd1dta'
               AND table_name LIKE 'ZDOEMP%' -- Data Migration - Ok/Error Table
...
spool "&SQL_LOG_PATH.\G1ZDMSTPOL_CALL_&log_date_text..txt"
...
--I_REMARKS: We can set any remark like the first cycle for Master Policy
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDMSTPOL';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Master Policy Testing by sql';
  BEGIN
BEGIN
SELECT table_name
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGMASPOL MP;
...
BEGIN
 ...
 Jd1dta.PV_MP_G1ZDMSTPOL(i_schedulename =>I_SCHEDULENAME,
                 i_schedulenumber => V_SCHEDULENUMBER);
END;
/
