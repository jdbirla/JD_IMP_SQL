/*
  PA Data Migration G1ZDMBRIND_CALL
*/
...
select 'G1ZDMBRIND' as SCHEDULENAME from dual;
...
spool "&SQL_LOG_PATH.\G1ZDMBRIND_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDMBRIND';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Member and Individual policy Testing by sql';
  BEGIN
BEGIN
SELECT table_name
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGMBRINDP1 MB;
/
...
BEGIN
 ...
 Jd1dta.PV_CH_G1ZDMBRIND(i_schedulename =>I_SCHEDULENAME,
                  i_schedulenumber => V_SCHEDULENUMBER);
END;
/
