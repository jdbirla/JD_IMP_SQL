/*
  PA Data Migration G1ZDPOLHST_CALL.sql
*/
...
select 'G1ZDPOLHST' as SCHEDULENAME from dual;
column schedulenumber new_value v_schedulenumber noprint

SELECT 
   lpad(substr(nvl(table_name, 'ZDOEPH0000'), 7, 4) + 1, 4, 0) as schedulenumber
      FROM (SELECT table_name
              FROM all_tables
             WHERE owner = 'Jd1dta'
               AND table_name LIKE 'ZDOEPH%'
             ORDER BY table_name DESC) a , dual b
     WHERE  b.dummy <> a.table_name (+) and 
     ROWNUM = 1;
...
spool "&SQL_LOG_PATH.\G1ZDPOLHST_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDPOLHST';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Policy Transaction History';
  BEGIN
BEGIN
SELECT nvl(table_name, 'ZDOEPH0000')
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGPOLTRNH PH;
/
...
-- Jd1dta.PV_CR_G1ZDPOLHST(i_schedulename =>I_SCHEDULENAME,
--                  i_schedulenumber => V_SCHEDULENUMBER);
END;
/
