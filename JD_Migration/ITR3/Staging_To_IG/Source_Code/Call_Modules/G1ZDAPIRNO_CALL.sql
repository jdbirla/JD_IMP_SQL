/*
  PA Data Migration G1ZDAPIRNO_CALL.sql
*/
...
select 'G1ZDAPIRNO' as SCHEDULENAME from dual;
column schedulenumber new_value v_schedulenumber noprint

SELECT 
   lpad(substr(nvl(table_name, 'ZDOERN0000'), 7, 4) + 1, 4, 0) as schedulenumber
      FROM (SELECT table_name
              FROM all_tables
             WHERE owner = 'Jd1dta'
               AND table_name LIKE 'ZDOERN%'
...
spool "&SQL_LOG_PATH.\G1ZDAPIRNO_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDAPIRNO';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Policy APIRNO';
  BEGIN
BEGIN
SELECT nvl(table_name, 'ZDOERN0000')
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGAPIRNO RN;
/
...
BEGIN
 ...
 --Jd1dta.PV_CR_G1ZDAPIRNO(i_schedulename =>I_SCHEDULENAME,
 --                 i_schedulenumber => V_SCHEDULENUMBER);
END;
/
