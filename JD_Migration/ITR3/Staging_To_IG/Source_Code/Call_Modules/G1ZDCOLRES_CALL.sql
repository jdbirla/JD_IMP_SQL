/*
  PA Data Migration G1ZDCOLRES_CALL.sql
*/
...
select 'G1ZDCOLRES' as SCHEDULENAME from dual;
...
spool "&SQL_LOG_PATH.\G1ZDCOLRES_CALL_&log_date_text..txt"
...
DECLARE
  ...
  I_SCHEDULENAME VARCHAR2(50 CHAR) := 'G1ZDCOLRES';
  I_ZPRVALDYN VARCHAR2(1 Char):= '&1';
  I_REMARKS VARCHAR2(500 CHAR):= 'Collection Result Testing';
  BEGIN
BEGIN
SELECT table_name
...
/
@DM2_ZDOE_REPORT.sql '&I_SCHEDULENAME' '&v_schedulenumber' TITDMGCOLRES CR;
/
...
BEGIN
 ...
 Jd1dta.PV_CR_G1ZDCOLRES(i_schedulename =>I_SCHEDULENAME,
                  i_schedulenumber => V_SCHEDULENUMBER);
END;
/
