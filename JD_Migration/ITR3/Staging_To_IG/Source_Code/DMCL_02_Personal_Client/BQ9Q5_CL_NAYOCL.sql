--------------------------------------------------------
--  File created - Monday-July-12-2021   
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure BQ9Q5_CL_NAYOCL
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "Jd1dta"."BQ9Q5_CL_NAYOCL" (i_scheduleName   IN VARCHAR2,
  ...
  *****************************************************************************************************/
  ...
  v_DM_OR_IG      view_dm_pazdnypf.DM_OR_IG%type;
  v_IS_UPDATE_REQ view_dm_pazdnypf.IS_UPDATE_REQ%type;
  ...
  C_Z098 CONSTANT VARCHAR2(4) := 'RQO6';
    /*CLTPHONE01 size is greater than 16*/
  ...
  CURSOR Cur_StageTable IS
    select A.*,b.zentity as zentity_dup
      from Jd1dta.DMIGTITNYCLT A
    left outer join PAZDNYPF  B
    on A.REFNUM = B.zentity 
    WHERE A.uniqueid between start_id and end_id
     order by LPAD(REFNUM, 8, '0') asc;

  obj_cur_Stagetab Cur_StageTable%rowtype;
  type t_nayose_list is table of Cur_StageTable%rowtype;
  nayose_list t_nayose_list;
BEGIN
  ...
  LOOP
    FETCH Cur_StageTable bulk collect
      INTO nayose_list limit C_limit;
    <<skipRecord>>
    for i in 1 .. nayose_list.count loop
      obj_cur_Stagetab := nayose_list(i);

      v_isAnyError := 'N';
      errorCount := 0;
      v_CLNTSTAS := null;
      v_DM_OR_IG := null;
      v_IS_UPDATE_REQ := null;
      i_zdoe_info := NULL;
      t_ercode(1) := ' ';
      t_ercode(2) := ' ';
      t_ercode(3) := ' ';
...
/*  NP2: start 
      ...
      END IF;
	   NP2: END 
    */  
       --CLTPHONE01 lengthb PA
      IF lengthb(RTRIM(obj_cur_Stagetab.dch_CLTPHONE01)) >16 THEN
        v_isAnyError := 'Y';
        errorCount := errorCount + 1;
        t_ercode(errorCount) := C_PA01;
        t_errorfield(errorCount) := 'CLTPHONE01';
        t_errormsg(errorCount) := 'CLTPHONE01 size is greater than 16';
        t_errorfieldval(errorCount) := TRIM(obj_cur_Stagetab.dch_CLTPHONE01);
        t_errorprogram(errorCount) := i_scheduleName;
        IF errorCount >= 5 THEN
          GOTO insertzdoe;
      ...
      IF v_isAnyError = 'N' AND i_zprvaldYN = 'N' THEN
        ...
        v_clntnum := SEQANUMPF.nextval; --PerfImprov
        --Insert Value Migration Registry Table
        obj_PAZDNYPF.PREFIX        := C_PREFIX;
        obj_PAZDNYPF.DM_OR_IG      := obj_cur_Stagetab.Dm_Or_Ig;
        obj_PAZDNYPF.CLNTSTAS      := obj_cur_Stagetab.Clntstas;
        obj_PAZDNYPF.IS_UPDATE_REQ := obj_cur_Stagetab.Is_Update_Req;
        obj_PAZDNYPF.ZENTITY       := obj_cur_Stagetab.refnum;
        if (obj_cur_Stagetab.Clntstas = C_NW) then
          obj_PAZDNYPF.ZIGVALUE := rpad(v_clntnum,8,' ') ;

        else
          obj_PAZDNYPF.ZIGVALUE := rpad( obj_cur_Stagetab.Ig_Clntnum,8,' ') ;
