...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure BQ9Q6_CL02_PERCLT
--------------------------------------------------------
set define off;

   CREATE OR REPLACE EDITIONABLE PROCEDURE "Jd1dta"."BQ9Q6_CL02_PERCLT" (i_scheduleName   IN VARCHAR2,
  ...
  * APR23    JDB       CP2      Nayose remove
  * OCT26  MKS     ZJNPG-8234 Update prover value for below columns:
                    cltaddr03, cltphone02, addrtype, idexpiredate, ispermanentid, workunit, zworkplce
  * JAN13    JDB       CP3          ITR3 Dev with Nayose changes                                       
  * NOV16    JDB       CP4      CLTPHONE01 is not mandatory   
...
v_SEQ_CLNTPF   clntpf.unique_number%type;
  ...
  ----------------CONSTANT:START------------- 
  C_PREFIX CONSTANT VARCHAR2(2) := Jd1dta.GET_MIGRATION_PREFIX('CLPL',
                                                               i_company);
  ...
  ------Constant----
  C_IDEXPIREDATE  CONSTANT NUMBER(8, 0) := 0; -- ZJNPG-8234: default value should be 0 not NULL.
  C_ISPERMANENTID CONSTANT CHAR(1 CHAR) := ' '; -- ZJNPG-8234: default value should be ' ' not NULL.
  C_SECUITYNO     CONSTANT CHAR(24 CHAR) := '                        ';
  C_PAYROLLNO     CONSTANT CHAR(10 CHAR) := '          ';
  C_SALUT         CONSTANT CHAR(6 CHAR) := null;
  C_CLTADDR03     CONSTANT NCHAR(50 CHAR) := '                                                  '; -- ZJNPG-8234: Value should be 50 spaces if ever data is not provided.
  ...
  C_WORKUNIT       CONSTANT NCHAR(60 CHAR) := '                                                            '; -- ZJNPG-8234: Value should be 60 spaces as default. 
  ...
  -- getnypf         pkg_common_dmcp.nypftype; ---CP2
  ...
  -- obj_nypf    pkg_common_dmcp.obj_nypf;--CP2
  ...
  CURSOR Cur_StageTable IS
    SELECT *
      FROM (SELECT perclnt.*,
                   ...
                   nypf.zentity,
                   nypf.zigvalue,
                   PACL.zentity  zentity_dup
              FROM (SELECT *
                      FROM (SELECT *
                              FROM Jd1dta.dmigtitdmgcltrnhis
                             WHERE transhist = '1') a
                      LEFT OUTER JOIN (SELECT postalcd,
                                             MIN(addrcd) AS minaddrcd
                                        FROM Jd1dta.zadrpf
                                       GROUP BY postalcd) post
                        ON TRIM(a.cltpcode) = post.postalcd) perclnt
              LEFT OUTER join Jd1dta.pazdnypf nypf
                ON perclnt.refnum = nypf.zentity
                LEFT OUTER JOIN (select * from PAZDCLPF where prefix = 'CP') PACL
                on perclnt.refnum =  PACL.zentity)
  ...
  type t_perclnt_list is table of Cur_StageTable%rowtype;
  perclnt_list t_perclnt_list;
BEGIN
  ...
  --pkg_common_dmcp.getnyval(getnypf => getnypf); ---CP2
 ...
 -- pkg_common_dmcp.checkcpdup(checkdupl => checkdupl);
  ...
  OPEN Cur_StageTable;
  LOOP
    FETCH Cur_StageTable bulk collect
      INTO perclnt_list limit C_limit;
    <<skipRecord>>
    for i in 1 .. perclnt_list.count loop
      obj_cur_Stagetab := perclnt_list(i);
     ...
     -- ELSIF (checkdupl.exists(TRIM(obj_cur_Stagetab.refnum))) THEN
           ELSIF (obj_cur_Stagetab.zentity_dup is not null) THEN
 ...
 */
      ...
      IF v_isAnyError = 'N' AND i_zprvaldYN = 'N' THEN
        v_clntnum :=rpad(obj_cur_Stagetab.zigvalue,8,' ') ;
        /* ---CP2
        ...
        -- obj_pazdclpf.RECSTATUS := obj_nypf.CLNTSTAS; --CP2
        ...
        obj_pazdclpf.PREFIX    := C_PREFIX;
        obj_pazdclpf.ZENTITY   := obj_cur_Stagetab.refnum;
        obj_pazdclpf.ZIGVALUE  := rpad(obj_cur_Stagetab.Zigvalue,8,' ');
        obj_pazdclpf.JOBNUM    := i_scheduleNumber;
        obj_pazdclpf.JOBNAME   := i_scheduleName;
        ...
        IF ((v_existing_Update = C_Y) or (v_new = C_Y)) THEN
          ...
          obj_clntpf.SURNAME       := RTRIM(obj_cur_Stagetab.lsurname);
          obj_clntpf.GIVNAME       := substr(obj_cur_Stagetab.lGIVNAME,1,20);
          obj_clntpf.SALUT         := C_SALUT;
          obj_clntpf.INITIALS      := SUBSTR(obj_cur_Stagetab.lgivname,
                                             1,
                                             1);
          ...
          --obj_clntpf.CLTADDR03     := obj_cur_Stagetab.CLTADDR03; ZJNPG-8234: value must be set to 50 spaces if data is not provided.
          ...
          --obj_clntpf.ADDRTYPE      := obj_cur_Stagetab.ADDRTYPE; ZJNPG-8234: value must be set to ' ' spaces if data is not provided.
          ...
          if (trim(obj_cur_Stagetab.CLTADDR03) is not null) then
            ...
            obj_clntpf.CLTADDR03 := C_CLTADDR03;
          end if;

          if (trim(obj_cur_Stagetab.ADDRTYPE) is not null) then
          ...
          --Start: ZJNPG-8234
          --obj_clntpf.ZWORKPLCE     := obj_cur_Stagetab.ZWORKPLCE;  --ZJNPG-8234: value must be set to 25 spaces if data is not provided.
          if (trim(obj_cur_Stagetab.ZWORKPLCE) is not null) then
            obj_clntpf.ZWORKPLCE := obj_cur_Stagetab.ZWORKPLCE;
          else
            obj_clntpf.ZWORKPLCE := '                         ';
          end if;
          --End: ZJNPG-8234
          ...
          ELSE
            Delete from CLNTPF where clntnum = v_clntnum;
            INSERT INTO CLNTPF VALUES obj_clntpf;
          END IF;
        END IF;
        ...
        IF ((v_existing_Update = C_Y) or (v_new = C_Y)) THEN
          ...
          v_pkValue := SEQ_CLEXPF.nextval; --PerfImprov
          ...
          ELSE
            Delete from CLEXPF where clntnum = v_clntnum;
            INSERT INTO CLEXPF VALUES obj_clexpf;
