CREATE OR REPLACE PROCEDURE "BQ9TV_CL02_2_CLNTHIST"(i_scheduleName   IN VARCHAR2,
  ...
  *****************************************************************************************************/
  ...
  v_clntstas       pazdnypf.clntstas%type;
    ...
    select *
      from (select *
              from (SELECT ROW_NUMBER() OVER(PARTITION BY refnum, effdate
                      ...
                      from (select *
                              from ((select * from Jd1dta.DMIGTITDMGCLTRNHIS) A left
                                    outer join
             ...
             where row_num = 1)
     WHERE refnum between start_id and end_id
     order by LPAD(REFNUM, 8, '0') asc, ZSEQNO asc;
  obj_cur_Stagetab personalclient_cursor%rowtype;
...
BEGIN
  ...
  LOOP
    ...
    If (trim(obj_client_old.refnum) is not null) then
      If (obj_client_old.cltdob != obj_cur_Stagetab.cltdob) then
        v_CLTDOBFLAG := 'Y';
      end if;
      If (obj_client_old.LSURNAME != obj_cur_Stagetab.LSURNAME) then
        v_LSURNAMEFLAG := 'Y';
      end if;
      If (obj_client_old.LGIVNAME != obj_cur_Stagetab.LGIVNAME) then
        v_LGIVNAMEFLAG := 'Y';
      end if;
      If (obj_client_old.zkanasurname != obj_cur_Stagetab.zkanasurname) then
        v_ZKANASNMFLAG := 'Y';
      end if;
      If (obj_client_old.zkanagivname != obj_cur_Stagetab.zkanagivname) then
        v_ZKANAGNMFLAG := 'Y';
      end if;
      If (obj_client_old.CLTSEX != obj_cur_Stagetab.CLTSEX) then
        v_CLTSEXFLAG := 'Y';
      end if;
      If (obj_client_old.CLTPCODE != obj_cur_Stagetab.CLTPCODE) then
        v_CLTPCODEFLAG := 'Y';
      end if;
      If (obj_client_old.ZKANADDR01 != obj_cur_Stagetab.ZKANADDR01) then
        v_ZKANADDR01FLAG := 'Y';
      end if;
      If (obj_client_old.ZKANADDR02 != obj_cur_Stagetab.ZKANADDR02) then
        v_ZKANADDR02FLAG := 'Y';
      end if;
      If (obj_client_old.CLTADDR01 != obj_cur_Stagetab.CLTADDR01) then
        v_CLTADDR01FLAG := 'Y';
      end if;
      If (obj_client_old.CLTADDR02 != obj_cur_Stagetab.CLTADDR02) then
        v_CLTADDR02FLAG := 'Y';
      end if;
      If (obj_client_old.CLTADDR03 != obj_cur_Stagetab.CLTADDR03) then
        v_CLTADDR03FLAG := 'Y';
      end if;
      ...
      If (obj_client_old.CLTPHONE02 != obj_cur_Stagetab.CLTPHONE02) then
        v_CLTPHONE02FLAG := 'Y';
      end if;
      If (obj_client_old.ZWORKPLCE != obj_cur_Stagetab.ZWORKPLCE) then
        v_ZWORKPLCEFLAG := 'Y';
      end if;
      If (obj_client_old.ZOCCDSC != obj_cur_Stagetab.ZOCCDSC) then
        v_ZOCCDSCFLAG := 'Y';
      end if;
    ...
    IF v_isAnyError = 'N' AND i_zprvaldYN = 'N' THEN
      ...
      IF (obj_cur_Stagetab.zseqno > 0) THEN
        ...
        obj_audit_clnt. newcltphone02 := obj_cur_Stagetab.cltphone02;
        ...
        IF ((TRIM(obj_client_old.cltphone01) !=
           TRIM(obj_cur_Stagetab.cltphone01))) THEN
          select SEQ_CLEXPF.nextval into v_pkauditclexpf from dual;
          obj_audit_clexp. unique_number := v_pkauditclexpf;
