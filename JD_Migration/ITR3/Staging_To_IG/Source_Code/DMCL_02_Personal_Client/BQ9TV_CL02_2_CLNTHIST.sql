...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure BQ9TV_CL02_2_CLNTHIST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "Jd1dta"."BQ9TV_CL02_2_CLNTHIST" (i_scheduleName   IN VARCHAR2,
   ...
   * APR23    JDB   CH2   Nayose Remove
   * SEP29    JDB   CH3   Remove validation and for effdate > business then error as per Jdrich will see later if any issue coming
                           in SHI it was there like if effdate > business date then assing business date into effdate
   * JAN15    JDB   CH4    ITR3 development with Nayose
  *****************************************************************************************************/
  ...
  v_refnum        Stagedbusr.TITDMGCLTRNHIS.REFNUM@DMSTAGEDBLINK%type;
  v_temprefnum    Stagedbusr.TITDMGCLTRNHIS.refnum@DMSTAGEDBLINK%type default ' ';
  isDateValid     VARCHAR2(20 CHAR);
  -- v_unq_zdch      PAZDCHPF.RECIDXCLNTHIS%type;
  --v_incrVersion   NUMBER(2) DEFAULT 0;
  v_occpclass    varchar2(2);
  v_pkauditclnpf number(18);
  v_pkazclnpf    number(18);
  v_busdate      busdpf.busdate%type;
  --v_clntstas       pazdnypf.clntstas%type;--CH2
  v_pkauditclexpf  number(18);
  v_CLTDOBFLAG     VARCHAR2(1) DEFAULT 'N';
  v_LSURNAMEFLAG   VARCHAR2(1) DEFAULT 'N';
  v_LGIVNAMEFLAG   VARCHAR2(1) DEFAULT 'N';
  v_ZKANASNMFLAG   VARCHAR2(1) DEFAULT 'N';
  v_ZKANAGNMFLAG   VARCHAR2(1) DEFAULT 'N';
  v_CLTSEXFLAG     VARCHAR2(1) DEFAULT 'N';
  v_CLTPCODEFLAG   VARCHAR2(1) DEFAULT 'N';
  v_ZKANADDR01FLAG VARCHAR2(1) DEFAULT 'N';
  v_ZKANADDR02FLAG VARCHAR2(1) DEFAULT 'N';
  v_ZKANADDR03FLAG VARCHAR2(1) DEFAULT 'N';
  v_ZKANADDR04FLAG VARCHAR2(1) DEFAULT 'N';

  v_CLTADDR01FLAG VARCHAR2(1) DEFAULT 'N';
  v_CLTADDR02FLAG VARCHAR2(1) DEFAULT 'N';
  v_CLTADDR03FLAG VARCHAR2(1) DEFAULT 'N';
  v_CLTADDR04FLAG VARCHAR2(1) DEFAULT 'N';

  v_CLTPHONE01FLAG VARCHAR2(1) DEFAULT 'N';
  v_CLTPHONE02FLAG VARCHAR2(1) DEFAULT 'N';
  v_ZWORKPLCEFLAG  VARCHAR2(1) DEFAULT 'N';
  v_OCCPCODEFLAG   VARCHAR2(1) DEFAULT 'N';
  v_ZOCCDSCFLAG    VARCHAR2(1) DEFAULT 'N';
  v_CLNTCHANGE     VARCHAR2(1) DEFAULT 'N';

  v_new             VARCHAR2(1);
  v_existing        VARCHAR2(1);
  v_existing_Update VARCHAR2(1);
  v_SEQ_VERSIONPF VERSIONPF.unique_number%type;

  ----------------Local Variables:END------------- 
  ----------------CONSTANT:START------------- 
  C_PREFIX CONSTANT VARCHAR2(2) := Jd1dta.GET_MIGRATION_PREFIX('CLHS',
                                                               i_company);

  C_DM_PA CONSTANT VARCHAR2(4) := 'DMPA';
  ...
  C_Z073 CONSTANT VARCHAR2(4) := 'RQNH'; --Not used curretly
  ...
  C_G844 CONSTANT VARCHAR2(4) := 'G844';
  ...
  /* CURSOR personalclient_cursor IS
    SELECT *
      FROM dmigvm1dta.TITDMGCLTRNHIS
  ...
  CURSOR personalclient_cursor IS
     SELECT *
      FROM (SELECT perclnt.*,
                   ...
                   nypf.zentity,
                   nypf.zigvalue,
                   CHPF.ZENTITY  ZENTITY_DUP
              from (select CLNTHIS.*,
                           ROW_NUMBER() OVER(PARTITION BY refnum

                           ORDER BY effdate, zseqno DESC) Z_New_SEQ
                      from (select *
                              from (SELECT ROW_NUMBER() OVER(PARTITION BY refnum, effdate
                                      ...
                                      from (select *
                                              from ((select *
                                                       from Jd1dta.DMIGTITDMGCLTRNHIS) A left
                                                    outer join
                                                    (select POSTALCD,
                                                            min(ADDRCD) as MINADDRCD
                                                       from Jd1dta.zadrpf
                                                      group by POSTALCD) POST on
                                                    trim(a.cltpcode) =
                                                    POST.POSTALCD)))
                             where row_num = 1) CLNTHIS) perclnt
              LEFT OUTER join Jd1dta.pazdnypf nypf
                ON perclnt.refnum = nypf.zentity
                LEFT OUTER JOIN Jd1dta.PAZDCHPF  CHPF
                on perclnt.refnum = CHPF.ZENTITY
                and perclnt.effdate = CHPF.effdate)
 WHERE REFNUMCHUNK between start_id and end_id
     order by LPAD(REFNUM, 8, '0') asc, Z_New_SEQ asc;

  obj_cur_Stagetab personalclient_cursor%rowtype;
  obj_client_old   personalclient_cursor%rowtype;

  type t_clnthis_list is table of personalclient_cursor%rowtype;
  clnthis_list t_clnthis_list;

BEGIN
  dbms_output.put_line('Start execution of BQ9TV_CL02_2_CLNTHIST, SC NO:  ' ||
  ...
  SELECT BUSDATE
    INTO v_busdate
    FROM Jd1dta.busdpf
   WHERE TRIM(company) = TRIM(i_company);

  -- Open Cursor
  OPEN personalclient_cursor;
  LOOP
    FETCH personalclient_cursor bulk collect
      INTO clnthis_list limit C_limit;
    <<skipRecord>>
    for i in 1 .. clnthis_list.count loop
      obj_cur_Stagetab := clnthis_list(i);
      ...
      v_existing        := 'N';
      v_existing_Update := 'N';
      v_clntnum         := obj_cur_Stagetab.Zigvalue;
      -----PA:Flag changes logic:start--------
      v_CLTDOBFLAG     := 'N';
      v_LSURNAMEFLAG   := 'N';
      v_LGIVNAMEFLAG   := 'N';
      v_ZKANASNMFLAG   := 'N';
      ...
      If (trim(obj_client_old.refnum) is not null) then
        If (rtrim(obj_client_old.cltdob) != rtrim(obj_cur_Stagetab.cltdob)) then
          v_CLTDOBFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.LSURNAME) !=
           rtrim(obj_cur_Stagetab.LSURNAME)) then
          v_LSURNAMEFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.LGIVNAME) !=
           rtrim(obj_cur_Stagetab.LGIVNAME)) then
          v_LGIVNAMEFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.zkanasurname) !=
           rtrim(obj_cur_Stagetab.zkanasurname)) then
          v_ZKANASNMFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.zkanagivname) !=
           rtrim(obj_cur_Stagetab.zkanagivname)) then
          v_ZKANAGNMFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTSEX) != rtrim(obj_cur_Stagetab.CLTSEX)) then
          v_CLTSEXFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTPCODE) !=
           rtrim(obj_cur_Stagetab.CLTPCODE)) then
          v_CLTPCODEFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.ZKANADDR01) !=
           rtrim(obj_cur_Stagetab.ZKANADDR01)) then
          v_ZKANADDR01FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.ZKANADDR02) !=
           rtrim(obj_cur_Stagetab.ZKANADDR02)) then
          v_ZKANADDR02FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTADDR01) !=
           rtrim(obj_cur_Stagetab.CLTADDR01)) then
          v_CLTADDR01FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTADDR02) !=
           rtrim(obj_cur_Stagetab.CLTADDR02)) then
          v_CLTADDR02FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTADDR03) !=
           rtrim(obj_cur_Stagetab.CLTADDR03)) then
          v_CLTADDR03FLAG := 'Y';
        end if;

        If (rtrim(replace(obj_client_old.CLTPHONE01, '-')) !=
           rtrim(replace(obj_cur_Stagetab.CLTPHONE01, '-'))) then
          dbms_output.put_line('Inside condition ');
          v_CLTPHONE01FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.CLTPHONE02) !=
           rtrim(obj_cur_Stagetab.CLTPHONE02)) then
          v_CLTPHONE02FLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.ZWORKPLCE) !=
           rtrim(obj_cur_Stagetab.ZWORKPLCE)) then
          v_ZWORKPLCEFLAG := 'Y';
        end if;
        If (rtrim(obj_client_old.ZOCCDSC) !=
           rtrim(obj_cur_Stagetab.ZOCCDSC)) then
          v_ZOCCDSCFLAG := 'Y';
        end if;

      end if;
      --CH4 Checking for client has been changed or not :START
      If (v_CLTDOBFLAG = 'Y' OR v_LSURNAMEFLAG = 'Y' OR
         v_LGIVNAMEFLAG = 'Y' OR v_ZKANASNMFLAG = 'Y' OR
         v_ZKANAGNMFLAG = 'Y' OR v_CLTSEXFLAG = 'Y' OR
         v_CLTPCODEFLAG = 'Y' OR v_ZKANADDR01FLAG = 'Y' OR
         v_ZKANADDR02FLAG = 'Y' OR v_ZKANADDR03FLAG = 'Y' OR
         v_ZKANADDR04FLAG = 'Y' OR v_CLTADDR01FLAG = 'Y' OR
         v_CLTADDR02FLAG = 'Y' OR v_CLTADDR03FLAG = 'Y' OR
         v_CLTADDR04FLAG = 'Y' OR v_CLTPHONE01FLAG = 'Y' OR
         v_CLTPHONE02FLAG = 'Y' OR v_ZWORKPLCEFLAG = 'Y' OR
         v_OCCPCODEFLAG = 'Y' OR v_ZOCCDSCFLAG = 'Y') THEN

        v_CLNTCHANGE := 'Y';
      ELSE
        v_CLNTCHANGE := 'N';
      END IF;
      --CH4 Checking for client has been changed or not : END

      -----Flag changes logic:END--------
      ...
      ELSE
       ...
       -- IF (checkdupl.exists(TRIM(v_refnum))) THEN
        IF(obj_cur_Stagetab.ZENTITY_DUP IS NOT NULL)THEN
          v_isAnyError                 := 'Y';
          i_zdoe_info.i_indic          := 'E';
      ...
      IF isDateValid <> 'OK' THEN
        ...
        END IF;
      END IF;
      ---CH3 : Start removing this validation
      /*
      IF (obj_cur_Stagetab.Effdate > v_busdate) THEN
        v_isAnyError := 'Y';
        errorCount := errorCount + 1;
        t_ercode(errorCount) := 'Z131';
        t_errorfield(errorCount) := 'effdate';
        t_errormsg(errorCount) := 'effdate > busdate';
        t_errorfieldval(errorCount) := TRIM(obj_cur_Stagetab.effdate);
        t_errorprogram(errorCount) := i_scheduleName;
        IF errorCount >= 5 THEN
          GOTO insertzdoe;
      ...
      */
      ---CH3 : END
      --- OCCPCODE valid in T-Table T3644
      -- Read T-Table T3644
      IF (TRIM(obj_cur_Stagetab.occpcode) IS NOT NULL) THEN
        IF NOT (itemexist.exists(TRIM(C_T3644) ||
                                 TRIM(obj_cur_Stagetab.occpcode) || 9)) THEN
          v_isAnyError := 'Y';
          errorCount := errorCount + 1;
      ...
      /* IF NOT (getzigvalue.exists(TRIM(v_refnum))) THEN
          v_isAnyError := 'Y';
          errorCount := errorCount + 1;
          t_ercode(errorCount) := C_G844;
          t_errorfield(errorCount) := 'REFNUM';
          t_errormsg(errorCount) := o_errortext(C_G844);
          t_errorfieldval(errorCount) := TRIM(v_refnum);
          t_errorprogram(errorCount) := i_scheduleName;
          IF errorCount >= 5 THEN
            GOTO insertzdoe;
          END IF;
      ...
      */
      /* --CH2
      IF (getzigvalue.exists(TRIM(v_refnum))) THEN

        v_clntstas := getzigvalue(TRIM(v_refnum)).Recstatus;

      end if;
      IF (v_clntstas = 'EX') THEN
        v_isAnyError := 'Y';
        errorCount := errorCount + 1;
        t_ercode(errorCount) := C_G844;
        t_errorfield(errorCount) := 'REFNUM';
      ...
      */
      ...
      --IG and NW not possible
      IF ((obj_cur_Stagetab.Dm_Or_Ig = C_IG) and
         (obj_cur_Stagetab.Clntstas = C_NW)) THEN
        v_isAnyError := 'Y';
        errorCount := errorCount + 1;
        t_ercode(errorCount) := C_PA01;
        t_errorfield(errorCount) := 'CLNTSTAS';
        t_errormsg(errorCount) := 'This combination is wrong';
        t_errorfieldval(errorCount) := TRIM(obj_cur_Stagetab.CLNTSTAS);
        t_errorprogram(errorCount) := i_scheduleName;
        IF errorCount >= 5 THEN
          GOTO insertzdoe;
        END IF;
      ...
      --IG and Y not possible
      IF ((obj_cur_Stagetab.Dm_Or_Ig = C_IG) and
         (obj_cur_Stagetab.Is_Update_Req = C_Y)) THEN
        v_isAnyError := 'Y';
        errorCount := errorCount + 1;
        t_ercode(errorCount) := C_PA01;
        t_errorfield(errorCount) := 'UPDATEREQ';
        t_errormsg(errorCount) := 'This combination is wrong';
        t_errorfieldval(errorCount) := TRIM(obj_cur_Stagetab.Is_Update_Req);
        t_errorprogram(errorCount) := i_scheduleName;
        IF errorCount >= 5 THEN
          GOTO insertzdoe;
        END IF;
      END IF;
      --CH4 
      If (TRIM(obj_cur_Stagetab.ZIGVALUE) IS NULL) THEN
        v_isAnyError                 := 'Y';
        i_zdoe_info.i_indic          := 'E';
      ...
      IF v_isAnyError = 'N' AND i_zprvaldYN = 'N' THEN
        ...
        IF (pitemexist.exists(trim(obj_cur_Stagetab.Occpcode))) THEN
          ...
          v_occpclass := pitemexist(trim(obj_cur_Stagetab.Occpcode))
                         .occclass;
        else
          v_occpclass := '  ';
        ...
        IF NOT (getzigvalue.exists(TRIM(v_refnum))) THEN
          CONTINUE skipRecord;
        ELSE
          v_clntnum := getzigvalue(TRIM(v_refnum)).zigvalue;
        END IF;
        */

        IF (obj_cur_Stagetab.Dm_Or_Ig = C_IG) THEN

          IF (obj_cur_Stagetab.Clntstas = C_EX) THEN
            v_existing := C_Y;
        ...
        ELSE
          IF (obj_cur_Stagetab.Clntstas = C_EX) THEN
            v_existing := C_Y;
            IF (obj_cur_Stagetab.Is_Update_Req = C_Y) THEN
              v_existing_Update := C_Y;

            ELSE
              v_existing_Update := C_N;
        ...
        -- DMLOG_INFO(v_clntnum||v_incrVersion,v_CLTPHONE01FLAG);

        -- SELECT SEQ_ZCLN_ZDCH.nextval INTO v_unq_zdch FROM dual;
        --Insert Value Migration Registry Table:start----
        -- SELECT "Jd1dta"."ISEQ$$_134747".currval INTO v_pkazclnpf FROM dual;
        ...
        obj_VIEW_DM_PAZDCHPF.ZCLNUNINO := v_pkazclnpf;
        ...
        ---New business 
        IF ((v_existing_Update = C_Y) or (v_new = C_Y)) THEN
          IF (obj_cur_Stagetab.Z_New_SEQ = C_ONE) THEN
            ------------Insert into Audit_CLNTPF : Start------------------------

            --SELECT SEQ_CLNTPF.nextval INTO v_pkauditclnpf FROM dual;
            v_pkauditclnpf := SEQ_CLNTPF.nextval; --Perfimprov
            obj_audit_clntpf.unique_number := v_pkauditclnpf;
            ...
            obj_audit_clntpf. newtranid := NVL(v_tranid, '              '); --ZJNPG-9095
            ...
            obj_audit_clntpf. newgivname := substr(obj_cur_Stagetab.lgivname,1,20);
            ...
            obj_audit_clntpf. newcltaddr03 := NVL(obj_cur_Stagetab.cltaddr03, ' '); --ZJNPG-9095
            ...
            obj_audit_clntpf. newmailing := C_MAILING;
            obj_audit_clntpf.newdirmail := C_DIRMAIL;
            obj_audit_clntpf. newaddrtype := obj_cur_Stagetab.addrtype;
            obj_audit_clntpf. newcltphone01 := obj_cur_Stagetab.cltphone01;
            obj_audit_clntpf. newcltphone02 := NVL(obj_cur_Stagetab.cltphone02, ' ');  --ZJNPG-9095
            obj_audit_clntpf. newvip := C_VIP;
            obj_audit_clntpf. newoccpcode := NVL(obj_cur_Stagetab.occpcode, '    '); --ZJNPG-9095
            obj_audit_clntpf. newservbrh := o_defaultvalues('SERVBRH');
            obj_audit_clntpf. newstatcode := C_STATCODE;
            obj_audit_clntpf. newcltdob := obj_cur_Stagetab.cltdob;
            obj_audit_clntpf. newsoe := C_SOE;
            obj_audit_clntpf. newdocno := C_DOCNO;
            ...
            obj_audit_clntpf. newtrtm := TO_CHAR(CURRENT_TIMESTAMP,
                                                 'YYMMDD');
            obj_audit_clntpf. newsndxcde := C_SNDXCDE;
            obj_audit_clntpf. newnatlty := C_NATLTY;
            obj_audit_clntpf. newfao := C_FAO;
            ...
            obj_audit_clntpf. newsrdate := o_defaultvalues('SRDATE');
            ...
            obj_audit_clntpf. newabusnum := C_ABUSNUM;
            obj_audit_clntpf. newbranchid := C_BRANCHID;
            ...
            obj_audit_clntpf. tranno := obj_cur_Stagetab.Z_New_SEQ;
            obj_audit_clntpf. systemdate := sysdate;
            obj_audit_clntpf. newoccpclas := v_occpclass;
            ...
            ELSE
              Delete from audit_clntpf where oldclntnum = v_clntnum;
              Insert into audit_clntpf values obj_audit_clntpf;
            ...
            --select SEQ_CLEXPF.nextval into v_pkauditclexpf from dual;
            v_pkauditclexpf := SEQ_CLEXPF.nextval; --PerfImprov
            obj_audit_clexp. unique_number := v_pkauditclexpf;
            ...
            obj_audit_clexp. newrdidtelno := C_RDIDTELNO;
            ...
            obj_audit_clexp. newrpager := C_RPAGER;
            obj_audit_clexp. newfaxno := C_FAXNO;
            obj_audit_clexp. newrinternet := C_RINTERNET;
            obj_audit_clexp. newrtaxidnum := C_RTAXIDNUM;
            obj_audit_clexp. newrstaflag := C_RSTAFLAG;
            obj_audit_clexp. newsplindic := C_SPLINDIC;
            obj_audit_clexp. newzspecind := C_ZSPECIND;
            obj_audit_clexp. newoldidno := C_OLDIDNO;
            ...
            obj_audit_clexp. tranno := obj_cur_Stagetab.Z_New_SEQ;
            ...
            ELSE
              Delete from audit_clexpf where oldclntnum = v_clntnum;
              insert into audit_clexpf values obj_audit_clexp;
            ...
            obj_versionpf.TRANNO  := obj_cur_Stagetab.Z_New_SEQ;
            obj_versionpf.CLNTNUM := v_clntnum;

            IF (v_new = C_Y) THEN
              INSERT INTO VERSIONPF VALUES obj_versionpf;
            ELSE
              Delete from VERSIONPF where clntnum = obj_versionpf.clntnum;
              INSERT INTO VERSIONPF VALUES obj_versionpf;
            END IF;
            ...
            ELSE
              Delete from VIEW_DM_ZCLNPF where clntnum = v_clntnum;
              INSERT INTO VIEW_DM_ZCLNPF VALUES obj_zclnpf;
          ...
          IF (obj_cur_Stagetab.Z_NEW_SEQ > C_ONE) THEN
            ...
            obj_versionpf.unique_number := v_SEQ_VERSIONPF;
              ...
              IF (v_new = C_Y) THEN
                INSERT INTO VERSIONPF VALUES obj_versionpf;
              ELSE
                Delete from VERSIONPF where clntnum = v_clntnum;
                INSERT INTO VERSIONPF VALUES obj_versionpf;
              END IF;
              ------------Insert into VERSIONPF : END------------------------
              ...
              ------------Insert into Audit_CLNTPF : Start------------------------
              --SELECT SEQ_CLNTPF.nextval INTO v_pkauditclnpf FROM dual;
              v_pkauditclnpf := SEQ_CLNTPF.nextval;
              obj_audit_clntpf.unique_number := v_pkauditclnpf;
              ...
              obj_audit_clntpf. oldgivname := substr(obj_client_old.lgivname,1,20);
              ...
              obj_audit_clntpf. oldmailing := C_MAILING;
              obj_audit_clntpf.olddirmail := C_DIRMAIL;
              ...
              obj_audit_clntpf. oldtrtm := TO_CHAR(CURRENT_TIMESTAMP,
                                                   'YYMMDD');
              obj_audit_clntpf. oldsndxcde := C_SNDXCDE;
              obj_audit_clntpf. oldnatlty := C_NATLTY;
              obj_audit_clntpf. oldfao := C_FAO;
              ...
              obj_audit_clntpf. oldsrdate := o_defaultvalues('SRDATE');
              ...
              obj_audit_clntpf. oldabusnum := C_ABUSNUM;
              obj_audit_clntpf. oldbranchid := C_BRANCHID;
              ...
              obj_audit_clntpf. oldoccpclas := v_occpclass;
              obj_audit_clntpf.OLDCLNTSTATECD := C_NULL;
              ...
              -----Insert into Audit_CLNT :START---------
              obj_audit_clnt. unique_number := v_pkauditclnpf;
              ...
              obj_audit_clnt. tranid := NVL(v_tranid, '              '); --ZJNPG-9095
              obj_audit_clnt. oldsurname := obj_client_old.lsurname;
              obj_audit_clnt. oldgivname := substr(obj_client_old.lgivname,1,20);
              ...
              obj_audit_clnt. oldcltaddr04 := C_CLTADDR04;
              obj_audit_clnt. oldcltaddr05 := C_CLTADDR05;
              obj_audit_clnt. oldclttyp := o_defaultvalues('CLTTYPE');
              obj_audit_clnt. oldctrycode := o_defaultvalues('CTRYCODE');
              obj_audit_clnt. oldmailing := C_MAILING;
              obj_audit_clnt. olddirmail := C_DIRMAIL;
              obj_audit_clnt. oldaddrtype := obj_client_old.addrtype;
              obj_audit_clnt. oldcltphone01 := obj_client_old.cltphone01;
              ...
              obj_audit_clnt. oldsalutl := C_SALUTL;
              -------------------NEW ----------------
              obj_audit_clnt. newsurname := obj_cur_Stagetab.lsurname;
              obj_audit_clnt. newgivname := substr(obj_cur_Stagetab.lgivname,1,20);
              obj_audit_clnt. newcltaddr01 := obj_cur_Stagetab.cltaddr01;
              obj_audit_clnt. newcltaddr02 := obj_cur_Stagetab.cltaddr02;
              obj_audit_clnt. newcltaddr03 := NVL(obj_cur_Stagetab.cltaddr03, ' '); --ZJNPG-9095
              obj_audit_clnt. newcltaddr04 := C_CLTADDR04;
              obj_audit_clnt. newcltaddr05 := C_CLTADDR05;
              obj_audit_clnt. newclttyp := o_defaultvalues('CLTTYPE');
              obj_audit_clnt. newctrycode := o_defaultvalues('CTRYCODE');
              obj_audit_clnt. newmailing := C_MAILING;
              obj_audit_clnt. newdirmail := C_DIRMAIL;
              obj_audit_clnt. newaddrtype := obj_cur_Stagetab.addrtype;
              obj_audit_clnt. newcltphone01 := obj_cur_Stagetab.cltphone01;
              obj_audit_clnt. newcltphone02 := NVL(obj_cur_Stagetab.cltphone02, ' ');  --ZJNPG-9095
              obj_audit_clnt. newcltdob := obj_cur_Stagetab.cltdob;
              obj_audit_clnt. newcltstat := o_defaultvalues('CLTSTAT');
              obj_audit_clnt. newsalutl := C_SALUTL;
              obj_audit_clnt. usrprf := i_usrprf;
              obj_audit_clnt. jobnm := i_scheduleName;
              ...
              ELSE
                Delete from audit_clnt where clntnum = v_clntnum;
                insert into audit_clnt values obj_audit_clnt;

              END IF;

              ----Insert into Audit_CLNT :END---------
              ...
              IF (v_CLTPHONE01FLAG = 'Y') THEN
                ...
                obj_audit_clexp. oldclntnum := v_clntnum;
                obj_audit_clexp. oldrdidtelno := C_RDIDTELNO;
                obj_audit_clexp. oldrmblphone := obj_client_old.cltphone01;
                obj_audit_clexp. oldrpager := C_RPAGER;
                obj_audit_clexp. oldfaxno := C_FAXNO;
                obj_audit_clexp. oldrinternet := C_RINTERNET;
                obj_audit_clexp. oldrtaxidnum := C_RTAXIDNUM;
                obj_audit_clexp. oldrstaflag := C_RSTAFLAG;
                obj_audit_clexp. oldsplindic := C_SPLINDIC;
                obj_audit_clexp. oldzspecind := C_ZSPECIND;
                obj_audit_clexp. oldoldidno := C_OLDIDNO;
              ...
              ELSE
                obj_zclnpf.OCCPCODE := C_ONESPACE;
              END IF;
              obj_zclnpf.OCCPCLAS := v_occpclass; ---need to check
              IF TRIM(obj_cur_Stagetab.zoccdsc) IS NOT NULL THEN
                obj_zclnpf.ZOCCDSC := obj_cur_Stagetab.zoccdsc;
              ELSE
                obj_zclnpf.ZOCCDSC := C_ONESPACE;
              ...
              obj_zclnpf.CLTDOBFLAG     := v_CLTDOBFLAG;
              obj_zclnpf.LSURNAMEFLAG   := v_LSURNAMEFLAG;
              obj_zclnpf.LGIVNAMEFLAG   := v_LGIVNAMEFLAG;
              obj_zclnpf.ZKANASNMFLAG   := v_ZKANASNMFLAG;
              obj_zclnpf.ZKANAGNMFLAG   := v_ZKANAGNMFLAG;
              obj_zclnpf.CLTSEXFLAG     := v_CLTSEXFLAG;
              obj_zclnpf.CLTPCODEFLAG   := v_CLTPCODEFLAG;
              obj_zclnpf.ZKANADDR01FLAG := v_ZKANADDR01FLAG;
              obj_zclnpf.ZKANADDR02FLAG := v_ZKANADDR02FLAG;
              obj_zclnpf.ZKANADDR03FLAG := v_ZKANADDR03FLAG;
              obj_zclnpf.ZKANADDR04FLAG := v_ZKANADDR03FLAG;
              obj_zclnpf.CLTADDR01FLAG  := v_CLTADDR01FLAG;
              obj_zclnpf.CLTADDR02FLAG  := v_CLTADDR02FLAG;
              obj_zclnpf.CLTADDR03FLAG  := v_CLTADDR03FLAG;
              obj_zclnpf.CLTADDR04FLAG  := v_CLTADDR04FLAG;
              obj_zclnpf.CLTPHONE01FLAG := v_CLTPHONE01FLAG;
              obj_zclnpf.CLTPHONE02FLAG := v_CLTPHONE02FLAG;
              obj_zclnpf.ZWORKPLCEFLAG  := v_ZWORKPLCEFLAG;
              obj_zclnpf.OCCPCODEFLAG   := v_OCCPCODEFLAG;
              obj_zclnpf.ZOCCDSCFLAG    := v_ZOCCDSCFLAG;
              obj_zclnpf.OCCPCLASFLAG   := C_N;
              obj_zclnpf.EFFDATE        := obj_cur_Stagetab.effdate;
              obj_zclnpf.DATIME         := CURRENT_TIMESTAMP;
              obj_zclnpf.JOBNM          := i_scheduleName;
  ...
  END LOOP;
  dbms_output.put_line('End execution of BQ9TV_CL02_2_CLNTHIST, SC NO:  ' ||
                       i_scheduleNumber || ' Flag :' || i_zprvaldYN);
...
exception
  WHEN OTHERS THEN
    ROLLBACK;
    p_exitcode := SQLCODE;
    p_exittext := 'BQ9TV_CL02_2_CLNTHIST : ' || i_scheduleName || ' ' ||
