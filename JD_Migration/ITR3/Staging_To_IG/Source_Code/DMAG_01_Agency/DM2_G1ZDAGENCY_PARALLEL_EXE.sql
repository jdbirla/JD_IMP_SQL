/**************************************************************************************************************************
  * File Name        : DM2_G1ZDAGENCY_PARALLEL_EXE
...
create or replace PROCEDURE                                           Jd1dta.DM2_G1ZDAGENCY_PARALLEL_EXE(i_schedulename   IN VARCHAR2,
...
BEGIN
 v_schedulenumber:= lpad(trim(i_schedulenumber), 4, '0' );
  p_exitcode := 0;
  p_exittext := NULL;
  dbms_output.put_line('Start execution of DM2_G1ZDAGENCY_PARALLEL_TASK');
  SELECT COUNT(1) INTO v_dataCount FROM Jd1dta.DMIGTITDMGAGENTPJ;

  IF (v_dataCount > 0) THEN
    SELECT COUNT(*)
      INTO v_taskCont
      FROM user_parallel_execute_tasks
     WHERE task_name = 'DM2_G1ZDAGENCY_PARALLEL_TASK';

    IF v_taskCont > 0 THEN
      dbms_parallel_execute.drop_task('DM2_G1ZDAGENCY_PARALLEL_TASK');
    END IF;
  ...
  */
    ...
    dbms_parallel_execute.create_task('DM2_G1ZDAGENCY_PARALLEL_TASK');
    dbms_parallel_execute.create_chunks_by_number_col('DM2_G1ZDAGENCY_PARALLEL_TASK',
                                                      'Jd1dta',
                                                      'DMIGTITDMGAGENTPJ',
                                                      'RECIDXAGENT',
                                                      i_chunksize);
    l_sql_stmt1 := 'BEGIN  Jd1dta.BQ9S5_AG01_AGENCY(';
    l_sql_stmt2 := '''' || i_schedulename || '''' || ',' || '''' ||
                   v_schedulenumber || '''' || ',' || '''' || i_zprvaldyn || '''' || ',' || '''' ||
                   i_company || '''' || ',' || '''' || i_usrprf || '''' || ',' || '''' ||
                   i_branch || '''' || ',' || 'NULL' || ',' || '''' ||
                   i_vrcmtermid || '''' || ',' || ':start_id' || ',' ||
                   ':end_id' || ');' || 'END;';

    l_sql_stmt_f := l_sql_stmt1 || l_sql_stmt2;
    dbms_output.put_line('Agency Procedure call :' || l_sql_stmt_f);

    dbms_parallel_execute.run_task(task_name      => 'DM2_G1ZDAGENCY_PARALLEL_TASK',
                                   sql_stmt       => l_sql_stmt_f,
                                   language_flag  => dbms_sql.native,
                                   parallel_level => i_degreeParallel); ---Keep this number low may be 2 or max 5.

    l_status := dbms_parallel_execute.task_status('DM2_G1ZDAGENCY_PARALLEL_TASK');

    o_status := l_status;

    dbms_output.put_line('DM2_G1ZDAGENCY_PARALLEL_TASK completed! Status:' ||
                         l_status);
  ...
  END IF;
     dbms_output.put_line('End execution of DM2_G1ZDAGENCY_PARALLEL_TASK');

exception
  WHEN OTHERS THEN
    ROLLBACK;
    p_exitcode := SQLCODE;
    p_exittext := 'DM2_G1ZDAGENCY_PARALLEL_EXE : ' ||
