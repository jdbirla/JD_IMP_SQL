create or replace PACKAGE "Jd1dta"."PKG_WRITE_XLSX" as
  /**************************************************************************************************************************
  * File Name        : PKG_WRITE_XLSX
  * Author           : Abhishek Gupta
  * Creation Date    : August 28, 2020
  * Project          : -----
  -----(formerly jdc Technologies India Private Limited)
  * Description      : This package contains all the common operations which are using for creating xlsx file
  **************************************************************************************************************************/
  ...
  function get_font(p_name      varchar2,
                    p_bold      boolean,
                    p_rgb       varchar2, -- this is a hex ALPHA Red Green Blue value
  ...
  function get_fill(p_patternType varchar2,
                    p_fgRGB       varchar2 -- this is a hex ALPHA Red Green Blue value
                    ) return pls_integer;
  --
  function get_border(p_top    varchar2,
                      p_bottom varchar2,
                      p_left   varchar2,
                      p_right  varchar2)
    return pls_integer;
  --
  function get_alignment(p_vertical   varchar2 := 'center',
                         p_horizontal varchar2 := 'center',
                         p_wrapText   boolean := false)
  ...
  procedure cell(p_col       pls_integer,
                 ...
                 p_alignment tp_alignment := get_alignment(),
  ...
  procedure query2sheet(p_sql            varchar2,
                        i_scheduleId     varchar2, 
                        p_column_headers boolean := true,
                        p_directory      varchar2 := null,
                        p_filename       varchar2 := null,
                        p_sheet          pls_integer := null);
-- Recon Set2
  procedure query2sheetrecon2(p_sql            varchar2,
                              p_batchid        varchar2, 
                              p_column_headers boolean := true,
                              p_directory      varchar2 := null,
                              p_filename       varchar2 := null,
...
/
create or replace PACKAGE BODY "Jd1dta"."PKG_WRITE_XLSX" as
  /**************************************************************************************************************************
  * File Name        : PKG_WRITE_XLSX
  * Author           : Abhishek Gupta
  * Creation Date    : August 28, 2020
  ...
  **************************************************************************************************************************/--
  ...
  begin
    ...
    if workbook.fonts.count() = 0 then
      t_ind := get_font('Calibri',false,null);
    end if;
    if workbook.fills.count() = 0 then
      t_ind := get_fill('none',null);
      t_ind := get_fill('gray125',null);
    end if;
  ...
  --
  function get_fill(p_patternType varchar2, p_fgRGB varchar2) return pls_integer is
    t_ind pls_integer;
  ...
  function get_alignment(p_vertical   varchar2 := 'center',
                         p_horizontal varchar2 := 'center',
                         p_wrapText   boolean := false) return tp_alignment is
    t_rv tp_alignment;
  begin
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="' ||
  ...
  procedure query2sheet(p_sql            varchar2,
    ...
    n_srcval  number := 0;
    n_statusval  varchar2(5);
    
    ---------------------------- Cursor for getting current date and time ----------------------------
    CURSOR cursor1 IS
    SELECT TO_CHAR(CURRENT_DATE, 'DD-Mon-YYYY HH:MI PM') as CURRENT_DATE_TIME FROM DUAL;
   obj_cursor1 cursor1%rowtype; 
  ...
  begin
    ...
    dbms_sql.describe_columns(t_c, t_col_cnt, t_desc_tab);
    
    -- bind variables to columns
    for c in 1..t_col_cnt
    loop
    case t_desc_tab(c).col_type
    when 1 then dbms_sql.define_column(t_c,c,v_val,4000);
    when 2 then dbms_sql.define_column(t_c,c,n_val);
    when 12 then dbms_sql.define_column(t_c,c,d_val);
    else
    dbms_sql.define_column(t_c,c,v_val,4000);
    end case;
    end loop;
    
    ---------------------------- Fill first 3 rows for reconciliation report info --------------------------------
    OPEN cursor1;
    FETCH cursor1 INTO obj_cursor1;
    cell(1, 1, 'Reconciliation Report', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null));
    cell(4, 1, 'Schedule Id', p_sheet => t_sheet);
    cell(5, 1, i_scheduleId, p_sheet => t_sheet);
    cell(1, 2, 'Generated on', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null));
    cell(3, 2, obj_cursor1.CURRENT_DATE_TIME, p_sheet => t_sheet);
    
    
    ------------------------------------ Fill headers in 4th row ------------------------------------
     for c in 1 .. t_col_cnt loop
      if p_column_headers then
       if (c = 1) then cell(c, 4, 'Recon Query ID', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 2) then cell(c, 4, 'Module', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 3) then cell(c, 4, 'Group Clause', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 4) then cell(c, 4, 'Where Clause', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4')); 
       end if;
       if (c = 5) then cell(c, 4, 'Validation', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 6) then cell(c, 4, 'DM/PJ Source Count', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 7) then cell(c, 4, 'Staging', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 8) then cell(c, 4, 'IG', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
       if (c = 9) then cell(c, 4, 'Result', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
       end if;
      end if;
    end loop;
    
    ------------------------------------ Fill excel data from 5th row ------------------------------------
    t_cur_row := case
        ...
        for c in 1 .. t_col_cnt loop
          case
            when t_desc_tab(c).col_type in (2, 100, 101) then
              dbms_sql.column_value(t_c, c, n_val);
              if (c = 6) then n_srcval := n_val;
              dbms_sql.column_value(t_c, 9, n_statusval);
              end if;
              if ((c = 7 OR c = 8) AND n_val != n_srcval AND UPPER(n_statusval) != UPPER('Pass')) THEN 
              cell(c, t_cur_row, n_val, p_sheet => t_sheet, p_borderId => get_border('thin','thin','thin','thin'), p_fontId => get_font('Calibri',false,'FFFF0000'));
              else
              cell(c, t_cur_row, n_val, p_sheet => t_sheet, p_borderId => get_border('thin','thin','thin','thin'));
              end if;
            when t_desc_tab(c).col_type in (12, 178, 179, 180, 181, 231) then
              dbms_sql.column_value(t_c, c, d_val);
               cell(c, t_cur_row, d_val, p_sheet => t_sheet, p_borderId => get_border('thin','thin','thin','thin'));
            when t_desc_tab(c).col_type in (1, 8, 9, 96, 112) then
              dbms_sql.column_value(t_c, c, v_val);
               cell(c, t_cur_row, v_val, p_sheet => t_sheet, p_borderId => get_border('thin','thin','thin','thin'));
            else
              null;
          end case;
        end loop;
 ...
 procedure query2sheetrecon2(p_sql            varchar2,
    ...
    n_srcval  number := 0;
    n_statusval  varchar2(5);
    lv_report_heading varchar2(100);
    lv_filename varchar2(50) := null;
    ---------------------------- Cursor for getting current date and time ----------------------------
    CURSOR cursor1 IS
    SELECT TO_CHAR(CURRENT_DATE, 'DD-Mon-YYYY HH:MI PM') as CURRENT_DATE_TIME FROM DUAL;
   obj_cursor1 cursor1%rowtype; 
  ...
  begin
   ...
   -- dbms_output.put_line('Inside the proc');

    t_c := dbms_sql.open_cursor;
   --   dbms_output.put_line('after open_cursor '||t_c);
   ...
   --  dbms_output.put_line('after parsing ');
    ...
    -- start execution of the SQL statement
    t_r := dbms_sql.execute(t_c);
    -- dbms_output.put_line('after execute '||t_r);
     
    -- get a description of the returned columns
    dbms_sql.describe_columns(t_c, t_col_cnt, t_desc_tab);

--dbms_output.put_line('after describe_columns '||t_col_cnt);

    -- bind variables to columns
    -- Settingup the column data types
    for c in 1..t_col_cnt
    loop
    case t_desc_tab(c).col_type
    when 1 then dbms_sql.define_column(t_c,c,v_val,4000);
    ...
    if p_batchid like 'RECON_DET%' 
    then
     -- dbms_output.put_line('If '||p_filename);
      lv_report_heading := 'Set2 Reconciliation Detail Report';
      lv_filename := p_filename;
    else
     -- dbms_output.put_line('Else '||p_filename);
      lv_report_heading := 'Set2 Reconciliation Summary Report';
      lv_filename := p_filename;
    end if;   
    
    cell(1, 1, lv_report_heading, p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null));
    cell(4, 1, 'Batch Id', p_sheet => t_sheet);
    cell(5, 1, p_batchid, p_sheet => t_sheet);
    cell(1, 2, 'Generated on', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null));
    cell(3, 2, obj_cursor1.CURRENT_DATE_TIME, p_sheet => t_sheet);
    

    
   --  dbms_output.put_line('before looping t_col_cnt '||t_col_cnt);

    ------------------------------------ Fill headers in 4th row ------------------------------------
     if p_batchid like 'RECON_DET%' then
         for c in 1 .. t_col_cnt loop
            if p_column_headers then
                 if (c = 1) then cell(c, 4, 'Detail Batch ID', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 2) then cell(c, 4, 'Policy number', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 3) then cell(c, 4, 'Product code', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 4) then cell(c, 4, 'Policy start date', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4')); 
                 end if;
                 if (c = 5) then cell(c, 4, 'Policy Status', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 6) then cell(c, 4, 'Effective date', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 7) then cell(c, 4, 'Effective Date Description', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 8) then cell(c, 4, 'Module', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 9) then cell(c, 4, 'Validating attributes', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 10) then cell(c, 4, 'Source Value', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 11) then cell(c, 4, 'Staging Value', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 12) then cell(c, 4, 'IG Value', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;
                 if (c = 13) then cell(c, 4, 'Summary Batch ID', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                 end if;       
            end if;
        end loop;
    else
   -- dbms_output.put_line('Else for Summary case '||t_col_cnt);
         for c in 1 .. t_col_cnt loop
            if p_column_headers then
                   if (c = 1) then cell(c, 4, 'Batch ID', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 2) then cell(c, 4, 'Module', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 3) then cell(c, 4, 'Validating attributes', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 4) then cell(c, 4, 'Source count', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4')); 
                   end if;
                   if (c = 5) then cell(c, 4, 'Staging count', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 6) then cell(c, 4, 'Mismatch between source and staging', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 7) then cell(c, 4, 'Staging count', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 8) then cell(c, 4, 'IG count', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
                   if (c = 9) then cell(c, 4, 'Mismatch between staging and IG', p_sheet => t_sheet, p_fontId => get_font('Calibri',true,null), p_borderId => get_border('thin','thin','thin','thin'), p_fillId => get_fill('solid','333175b4'));
                   end if;
           end if;
        end loop;
    end if;
   ...
   -- dbms_output.put_line('t_c '||t_c);

    loop
      t_r := dbms_sql.fetch_rows(t_c);
     -- dbms_output.put_line('t_r '||t_r);
      exit when t_r = 0;
        for c in 1 .. t_col_cnt loop
    ...
    --dbms_output.put_line('Before saving '||lv_filename);
    
    if (p_directory is not null and lv_filename is not null) then
  ...
  Jd1dta.insert_error_log(sqlcode,sqlerrm,'query2sheetrecon2');   
      if dbms_sql.is_open(t_c) then
        dbms_sql.close_cursor(t_c);
      end if;
  end;
