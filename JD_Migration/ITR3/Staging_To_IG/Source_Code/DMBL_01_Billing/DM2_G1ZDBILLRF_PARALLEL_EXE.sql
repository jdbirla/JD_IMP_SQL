/**************************************************************************************************************************
  * File Name        : DM2_G1ZDBILLRF_PARALLEL_EXE
  * Author           : Jitendra Birla
  * Creation Date    : March 16, 2020
  * Project          : -----
  -----(formerly jdc Technologies India Private Limited)
  * Description      : This Procedure is for parallel run for Agency module
  **************************************************************************************************************************/
   /***************************************************************************************************
...
create or replace PROCEDURE Jd1dta.DM2_G1ZDBILLRF_PARALLEL_EXE(i_schedulename   IN VARCHAR2,
  ...
  C_PREFIX CONSTANT VARCHAR2(2 CHAR) := GET_MIGRATION_PREFIX('RFBL',i_company);
  p_exitcode number;
...
BEGIN
 v_schedulenumber:= lpad(trim(i_schedulenumber), 4, '0' );
  p_exitcode := 0;
  p_exittext := NULL;
  dbms_output.put_line('Start execution of DM2_G1ZDBILLRF_PARALLEL_TASK');
  SELECT COUNT(1) INTO v_dataCount FROM Jd1dta.DMIGTITDMGREF1;
  ...
  IF (v_dataCount > 0) THEN
    SELECT COUNT(*)
     ...
     WHERE task_name = 'DM2_G1ZDBILLRF_PARALLEL_TASK';

    IF v_taskCont > 0 THEN
      dbms_parallel_execute.drop_task('DM2_G1ZDBILLRF_PARALLEL_TASK');
    END IF;
    ...
    dbms_parallel_execute.create_task('DM2_G1ZDBILLRF_PARALLEL_TASK');
    dbms_parallel_execute.create_chunks_by_number_col('DM2_G1ZDBILLRF_PARALLEL_TASK',
                                                      'Jd1dta',
                                                      'DMIGTITDMGREF1',
                                                      'REFNUMCHUNK',
                                                      i_chunksize);
    ...
    l_sql_stmt1 := 'BEGIN  Jd1dta.BQ9UX_BL01_REFUNDBL(';
	
	dbms_output.put_line('Bill Refund Procedure call :' || l_sql_stmt1);
    ...
    l_sql_stmt2 := '''' || i_schedulename || '''' || ',' || '''' ||
                   ...
                   i_company || '''' || ',' || '''' || i_usrprf || '''' || ',' 
				   || '''' || i_branch || '''' || ',' 
				   || '''' || i_transcode|| '''' || ',' || i_vrcmtime || ',' || i_vrcmuser|| ',' ||i_acctYear || ',' || i_acctMonth || ',' 
				   || '''' || i_vrcmtermid || '''' || ',' 
				    || '''' || i_array_size || '''' || ',' || ':start_id' || ',' ||
                   ':end_id' || ');' || 'END;';
	dbms_output.put_line('Bill Refund Procedure call :' || l_sql_stmt2);

    l_sql_stmt_f := l_sql_stmt1 || l_sql_stmt2;
    dbms_output.put_line('Bill Refund Procedure call :' || l_sql_stmt_f);

    dbms_parallel_execute.run_task(task_name      => 'DM2_G1ZDBILLRF_PARALLEL_TASK',
                                   sql_stmt       => l_sql_stmt_f,
                                   language_flag  => dbms_sql.native,
                                   parallel_level => i_degreeParallel); ---Keep this number low may be 2 or max 5.

    l_status := dbms_parallel_execute.task_status('DM2_G1ZDBILLRF_PARALLEL_TASK');

    o_status := l_status;

    dbms_output.put_line('DM2_G1ZDBILLRF_PARALLEL_TASK completed! Status:' || l_status);
	
  END IF;
     dbms_output.put_line('End execution of DM2_G1ZDBILLRF_PARALLEL_TASK');

exception
  WHEN OTHERS THEN
    ROLLBACK;
    p_exitcode := SQLCODE;
    p_exittext := 'DM2_G1ZDBILLRF_PARALLEL_EXE : ' ||
