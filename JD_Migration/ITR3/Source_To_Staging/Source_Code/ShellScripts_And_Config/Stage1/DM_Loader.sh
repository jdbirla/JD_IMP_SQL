...
tr -d $'\r' < $DM_inputcsvpath/$DM_tablename.CSV > temp_file.CSV
if [ $? -ne 0 ]
then
  echo "CONTROL M character removal fail !!! ">> $1"_loader.log"
  exit 1
fi
mv temp_file.CSV $DM_inputcsvpath/$DM_tablename.CSV
if [ $? -ne 0 ]
then
...
sqlldr USERID=$DM_dbconnection  CONTROL=$DM_ctlfilepath/$DM_tablename.ctl DIRECT=TRUE ROWS=500 ERRORS=1000 LOG=$DM_logfilepath/$DM_tablename$run_time.log BAD=$DM_badfilepath/$DM_tablename$run_time.bad DATA=$DM_inputcsvpath/$DM_tablename.CSV DISCARD=$DM_dicfilepath/$DM_tablename.dis
...
case "$retcode" in 
    0 ) 
         ...
         awk -v var=`echo "$DM_tablename.CSV"|sed s/^TMP_//g` '{if($1==var) {$2="F"} print $0}' $DM_inputfilepath/DM_Inputlist_stg1.txt >test3.txt
         ...
         then
              ...
              if [ "$v_bad" -ne 0 ]
              then
                  #awk -v var="$DM_tablename.CSV" '{if($1==var) {$2="F"} print $0}' $DM_inputfilepath/DM_Inputlist_stg1.txt >test3.txt
                  awk -v var=`echo "$DM_tablename.CSV"|sed s/^TMP_//g` '{if($1==var) {$2="F"} print $0}' $DM_inputfilepath/DM_Inputlist_stg1.txt >test3.txt
	          ...
	          echo " BAD Records found while loading data into staging table" >> $1"_loader.log"
                  ...
                  #awk -v var="$DM_tablename.CSV" '{if($1==var) {$2="S"} print $0}' $DM_inputfilepath/DM_Inputlist_stg1.txt >test3.txt
                  awk -v var=`echo "$DM_tablename.CSV"|sed s/^TMP_//g` '{if($1==var) {$2="S"} print $0}' $DM_inputfilepath/DM_Inputlist_stg1.txt >test3.txt
                  mv test3.txt $DM_inputfilepath/DM_Inputlist_stg1.txt
                  echo " SQL Loader successfully loaded into staging table" >> $1"_loader.log"
              fi ;; 
    1 )    
