...
echo "MEMBER POLICY  REPORT    Stage 4 Movement " >> MEMBERPOLICY_REPORT.LOG
echo "SOURCE TABLE(s) : TITDMGMBRINDP1" >>MEMBERPOLICY_REPORT.LOG
...
SET FEEDBACK OFF
SET TAB ON
--execute DM_data_bulkcopy_IG.DM_Memberpol_to_ig($DM_IGSchema,1000,'Y');
execute DM_bulkcopy_mempol.DM_Memberpol_to_ig($DM_IGSchema,1000,'Y');
PROMPT TARGET_TABLE    INPUT_CNT  OUTPUT_CNT STATUS ERRORMSG ;
PROMPT ============    =========  ========== ====== ========;
SELECT LPAD(REPLACE(TARGET_TABLE,'_IG',''),15,' ')||lpad(to_char(INPUT_CNT),10,' ')||LPAD(to_char(OUTPUT_CNT),10,' ')||LPAD(STATUS,6,' ')||'      ' || ERROR_MSG FROM IG_COPY_CNTL_TABLE WHERE TARGET_TABLE='TITDMGMBRINDP1_IG';
EXIT
EOF
retcd=$?
...
cat MEMBERPOLICY_REPORT.LOG > "${DM_logfilepath}/STAGE4_MEMBERPOLICY_REPORT${timeStamp}.LOG"
