...
/
/***************** Pipeline approach*******************************************/
create or replace function APM_DBMASK_TITDMGMBRINDP1_pip return  APM_DBMASK_TITDMGMBRINDP1_tab pipelined
PARALLEL_ENABLE
as
  begin
  for idx in (
 ...
 BNKACCKEY01	
  ...
  from TITDMGMBRINDP1@stagedblink fetch first 100 percent rows only 
...
*******************************************************************************/
create or replace procedure APM_DBMASKwritetoTITDMGMBRP1(p_dir in VARCHAR2, p_fn in varchar2 )  as
  v_file  UTL_FILE.FILE_TYPE;
  v_sql_res   VARCHAR2(4000);
begin
  v_file := UTL_FILE.FOPEN(p_dir, p_fn, 'w',32767);
  for i in (select/*+parallel(4)*/ * from table(APM_DBMASK_TITDMGMBRINDP1_pip)) loop
...
exec APM_DBMASKwritetoTITDMGMBRP1('IMP_DATA_DIR','TITDMGMBRINDP1.csv');
