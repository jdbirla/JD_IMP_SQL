create or replace type APM_DBMASK_TITDMGCLPRSN_obj as object(
...
/  
create or replace type APM_DBMASK_TITDMGCLPRSN_tab as table of APM_DBMASK_TITDMGCLPRSN_obj;
/
/***************** Pipeline approach*******************************************/
create or replace function APM_DBMASK_TITDMGCLPRSN_pip return  APM_DBMASK_TITDMGCLPRSN_tab pipelined
PARALLEL_ENABLE
as
  begin
  for idx in (
 ...
 CLTPHONE01   ,
 CLTPHONE02   
from TITDMGCLNTPRSN@stagedblink fetch first 100 percent rows only 
  )loop
    pipe row(
      APM_DBMASK_TITDMGCLPRSN_obj( 
 idx.UNIQUE_NUMBER	,
 idx.LSURNAME     ,
 idx.LGIVNAME     ,
 idx.ZKANAGIVNAME ,
...
*******************************************************************************/
create or replace procedure APM_DBMASKwritetoTITDMGCLPRSN(p_dir in VARCHAR2, p_fn in varchar2 )  as
  v_file  UTL_FILE.FILE_TYPE;
  v_sql_res   VARCHAR2(4000);
begin
  v_file := UTL_FILE.FOPEN(p_dir, p_fn, 'w',32767);
  for i in (select/*+parallel(4)*/ * from table(APM_DBMASK_TITDMGCLPRSN_pip)) loop
...
--exec APM_DBMASKwritetoTITDMGCLPRSN('IMP_DATA_DIR','TITDMGCLPRSN.csv');
